...
t_inst	**sort_chunk_up(t_list **lsta, t_list **lstb, t_inst **insts)//24
{
	int	save;
	int	count;
...
t_inst	**sort_chunk_down(t_list **lsta, t_list **lstb, t_inst **insts, int chunk)
{
	...
	range = sort_chunk_range(lstb, range, chunk);
	if ((*lsta)->value > (*lstb)->value)
		return (inst_pb(lsta, lstb, insts, 1));
	else 
		return (inst_pb(lsta, lstb, insts, 1), inst_sb(lstb, insts, 1));
}
...
t_inst	**sort_chunk(t_list **lsta, t_list **lstb, t_inst **insts, int chunk)
{
	...
	if ((*lstb)->next == NULL)
		return (inst_pb(lsta, lstb, insts, 1));
	if (num == chunk || num == chunk + 1)
		return (sort_chunk_up(lsta, lstb,insts));
	return (sort_chunk_down(lsta, lstb, insts, chunk));
}