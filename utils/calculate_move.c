...
t_inst	**both_rra(t_list **lsta, t_list **lstb, t_inst **insts, t_range range)
{
	...
	else if (range.low == 2147483647)
	{
		if (range.m_pos == range.h_pos)
			...
			return (inst_rrr(lsta, lstb, insts, range.h_pos));
		return (inst_rra(lsta, insts, range.h_pos - range.m_pos));
	}
}
...
t_inst	**ch_move(t_list **lsta, t_list **lstb, t_inst **insts, t_range range)
{
	if (range.high == 2147483647)
	{
		if (range.m_pos && range.m_pos < (range.mid / 2))
			if (range.l_pos < (range.mid / 2))
				return (both_ra(lsta, lstb, insts, range));
		if (range.m_pos && range.m_pos > (range.mid / 2))
			if (range.l_pos > (range.mid / 2))
				return (both_rra(lsta, lstb, insts, range));
		else if (range.l_pos < (range.mid / 2))
			return (inst_ra(lsta, insts, range.l_pos));
		return (inst_rra(lsta, insts, range.l_pos));
	}
	else if (range.low == 2147483647)
	{
		if (range.m_pos && range.m_pos < (range.mid / 2))
			if (range.h_pos < (range.mid / 2))
				return (both_ra(lsta, lstb, insts, range));
		if (range.m_pos && range.m_pos > (range.mid / 2))
			if (range.h_pos > (range.mid / 2))
				return (both_rra(lsta, lstb, insts, range));
		else if (range.h_pos < (range.mid / 2))
			return (inst_ra(lsta, insts, range.h_pos));
		return (inst_rra(lsta, insts, range.h_pos));
	}
}

t_inst	**cal_move(t_list **lsta, t_list **lstb, t_inst **insts, t_range range)//18
{
